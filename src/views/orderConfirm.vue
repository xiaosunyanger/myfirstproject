<template>
    <div>
        ...
        <div class="order-foot-wrap">
            ...
            <router-link to="/address" tag="button" class="btn btn--m">上一步</router-link>
            </div>
            <div class="next-btn-wrap">
                <button class="btn btn--m btn--red" @click="payment">去结算</button>
            </div>
        </div>
        </div>
      </div>
        ...
        //获取订单页面
         ...
         axios.get("/users/cartList").then((response)=>{
             ...
             if(res.status=='0'){
                ...
                (item.checked==1)&&(this.subtotal+=item.salePrice*item.productNum);
                 })
                 //税后价钱计算
                this.orderTotal=this.subtotal+this.tax-this.discount+this.shipping;
             }else{
                   this.cartList=[];
             }
         })
